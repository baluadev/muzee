...
Future<T?> pushScreenWithNavBar<T>(BuildContext context, Widget screen) {
  ...
  FirebaseAnalyticsService.inst.logEvent(
    'open_screen',
    data: {'screen_name': screenName},
  );
  return pushScreen<T>(context, screen: screen, withNavBar: true);
}
...
Future<T?> pushScreenWithoutNavBar<T>(BuildContext context, Widget screen) {
  ...
  return pushScreen<T>(context, screen: screen);
}
