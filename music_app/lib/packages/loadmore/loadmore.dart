...
// import 'package:ttvl/configs/colors.gen.dart';
// import 'package:ttvl/helper/locale_support.dart';
...
class _LoadMoreState extends State<LoadMore> {
  ...
  Widget build(BuildContext context) {
    ...
    if (child is GridView) {
      return ValueListenableBuilder(
        valueListenable: _loadMoreStatus,
        builder: (context, value, child) {
          return _buildGridView(widget.child as GridView) ?? Container();
        },
      );
    }
  ...
  Widget? _buildGridView(GridView gridView) {
    ...
    if (delegate is SliverChildBuilderDelegate) {
      if (!widget.whenEmptyLoad && (delegate.estimatedChildCount ?? 0) == 0) {
        return gridView;
      }
      final itemCount = (delegate.estimatedChildCount ?? 0) + 1;
    ...
    if (delegate is SliverChildListDelegate) {
      ...
      final children = List<Widget>.from(delegate.children);
      children.add(_buildLoadMoreView());
      return GridView.count(
        crossAxisCount: _getCrossAxisCount(gridView.gridDelegate),
        shrinkWrap: gridView.shrinkWrap,
        controller: gridView.controller,
        physics: gridView.physics,
        padding: gridView.padding,
  ...
  int _getCrossAxisCount(SliverGridDelegate delegate) {
...
String _buildVietnameseText(LoadMoreStatus status) {
  ...
  switch (status) {
    case LoadMoreStatus.fail:
      text = 'Tải thất bại, vui lòng thử lại!';
      break;
    case LoadMoreStatus.idle:
      text = 'Đang tải...';
      break;
